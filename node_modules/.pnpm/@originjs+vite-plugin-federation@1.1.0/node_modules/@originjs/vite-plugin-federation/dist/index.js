...
function normalizePath(id) {
    return path__namespace.posix.normalize(id.replace(/\\/g, '/'));
}
function isSameFilepath(src, dest) {
    ...
    const srcExt = path__namespace.parse(src).ext;
    const destExt = path__namespace.parse(dest).ext;
        ...
        async transform(code, id) {
            ...
            if (builderInfo.isShared) {
                ...
                if (id === '\0virtual:__federation_fn_import') {
                    const moduleMapCode = parsedOptions.prodShared
                        .map((sharedInfo) => `'${removeNonLetter(sharedInfo[0])}':{get:()=>__federation_import('./${path__namespace.basename(this.getFileName(sharedInfo[1].emitFile))}'),import:${sharedInfo[1].import}${sharedInfo[1].requiredVersion
                ...
                if (id === '\0virtual:__federation_lib_semver') {
                    ...
                    const satisfyId = `${path__namespace.dirname(federationId)}/satisfy.js`;
                    return fs__namespace.readFileSync(satisfyId, { encoding: 'utf-8' });
                }
            }
            if (builderInfo.isRemote) {
                ...
                if (id === '\0virtual:__remoteEntryHelper__') {
                    for (const expose of parsedOptions.prodExpose) {
                        code = code.replace(`\${__federation_expose_${expose[0]}}`, `./${path__namespace.basename(this.getFileName(expose[1].emitFile))}`);
                    }
                    return code;
                }
            }
                ...
                if (id === '\0virtual:__federation__') {
                    ...
                    parsedOptions.prodShared.forEach((arr) => {
                        ...
                        if (typeof obj === 'object') {
                            const fileName = `./${path__namespace.basename(this.getFileName(obj.emitFile))}`;
    ...
    return {
      ...
      `
        ...
        renderChunk: function (code, chunk, options) {
            ...
            const sharedFlag = sharedFileReg.test(path__namespace.basename(chunk.fileName));
            ...
            const needSharedImport = isRemote &&
                ...
                chunk.imports.some((importName) => sharedFileReg.test(path__namespace.basename(importName)));
            if (needSharedImport) {
                            ...
                            walk(ast, {
                                enter(node) {
                                    ...
                                    if (node.type === 'ImportDeclaration' &&
                                        sharedFileReg.test(path__namespace.basename(node.source.value))) {
                                        const sharedName = (_a = path__namespace
                                    ...
                                    if (expression) {
                                        ...
                                        if (args[0].type === 'ArrayExpression' &&
                                            ...
                                            chunk.imports.forEach((importName, index) => {
                                                var _a;
                                                const baseName = path__namespace.basename(importName);
...
function prodExposePlugin(options) {
    ...
    for (const item of parsedOptions.prodExpose) {
        ...
        const exposeFilepath = normalizePath(path__namespace.resolve(item[1].import));
        ...
        generateBundle(_options, bundle) {
            ...
            if (remoteEntryChunk) {
                ...
                moduleCssFileMap.forEach((value, key) => {
                    item.code = item.code.replace(`${DYNAMIC_LOADING_CSS_PREFIX}${key}`, `./${path__namespace.basename(value)}`);
                });
    ...
    function getChunkCssRelation(bundle) {
        ...
        for (const file in bundle) {
            if (path__namespace.extname(file) === '.css') {
                cssFileMap.set(getOriginalFileName(file), file);
            }
        }
        for (const file in bundle) {
            let name = getOriginalFileName(file);
            if (cssFileMap.get(name) != null && path__namespace.extname(file) !== '.css') {
        ...
        function getOriginalFileName(file) {
            return path__namespace.parse(path__namespace.parse(file).name).name;
        }
    }
}
